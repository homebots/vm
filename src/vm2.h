...
const uint8_t c_debug = 0xfb;          // uint8_t enable
...
void vm_reset(Program *program);
void vm_yield(Program *program, uint8_t time);
void vm_cycle(Program *program);
// void vm_tick(Program *program, uint32_t delay);

uint8_t read_argument(Program *program, uint8_t index)
{
  return *((volatile uint8_t *)(vm->arguments[index]));
}

uint32_t read_argument32(Program *program, uint8_t index)
{
...
void ICACHE_FLASH_ATTR vm_fcycle(Program *program)
{
  uint8_t next = vm->program[vm->cursor];
  if (next == c_halt)
  {
    vm_advance(vm, 1);
  }
...
void ICACHE_FLASH_ATTR vm_cycle(Program *program)
{
  uint8_t next = vm->program[vm->cursor];
  ...
  case c_yield:
    arg_byte(vm, 0);
    vm_yield(vm, read_argument(vm, 0));
    return;
  ...
  case c_inc:
    arg_byte(vm, 0); // slot
    LOG("inc [%d]\n", read_argument(vm, 0));
    (uint32_t) vm->slot[read_argument(vm, 0)]++;
    break;
  ...
  case c_jump:
    ...
    copy_int32(&vm->cursor, vm->arguments[0]);
    LOG("jump %ld\n", vm->cursor);
    break;
  ...
  case c_jumpif:
    ...
    {
      copy_int32(&vm->cursor, vm->arguments[1]);
      LOG("jumpif [%d], %ld\n", read_argument(vm, 0), vm->cursor);
    }
    break;
...
os_timer_setfn(&vm->delay, (os_timer_func_t *)&vm_cycle, vm);

void vm_init(Program *program)
{
  vm_reset(vm);
}

void vm_load(Program *program, uint8_t *program, int length)
{
  if (vm->program != 0)
    // {
    os_free(vm->program);
