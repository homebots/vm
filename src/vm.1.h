...
void vm_yield(EspVM *vm, uint8_t time);
void vm_cycle(EspVM *vm);
void vm_tick(EspVM *vm, uint32_t delay);
...
void ICACHE_FLASH_ATTR vm_dump_state(EspVM *vm)
{
  int i;
  LOG("PC %ld\n", vm->programCounter);
  LOG("Slots\n");
...
void ICACHE_FLASH_ATTR vm_yield(EspVM *vm, uint8_t time)
{
  os_delay_us((uint16_t)time);
  vm_cycle(vm);
}
